...
class SchedulePage extends React.Component {
    ...
    fetch(
      ...
      .then(getscheduleResponse => {
        ...
        if (getscheduleStatus === 200 && getscheduleResponse.body.state === 1) {
          ...
          if (getscheduleResponse.body.schedule.length >= 1) {
            var schedule_dates = [];
            var day_tmp =
              getscheduleResponse.body.schedule[0].Start_Date * 1000;
            for (
              var i = 0;
              i < getscheduleResponse.body.schedule[0].Duration;
              i++
            ) {
          ...
          this.setState({
            schedule: getscheduleResponse.body.schedule,
            schedule_dates,
          });
          console.log('schedule_dates', schedule_dates);
        }
      })
      .catch(err => {
        console.log(err);
  ...
  render() {
    ...
    return (
      <Page className="SchedulePage" title="Treatment Calendar">
        <Row>
