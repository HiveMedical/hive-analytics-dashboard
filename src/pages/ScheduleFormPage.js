...
function ScheduleFormPage() {
      ...
      if (event.target.id[index] === 'S') {
        setState({
          ...
          Regimen_Week: {
            ...
            [day]: {
              startTime: value,
              endTime: state.Regimen_Week[day].endTime
                ? state.Regimen_Week[day].endTime
                : '',
            },
          },
        });
        ...
        setState({
          ...
          Regimen_Week: {
            ...state.Regimen_Week,
            [day]: {
              startTime: state.Regimen_Week[day].startTime
                ? state.Regimen_Week[day].startTime
                : '',
              endTime: value,
            },
          },
  ...
  const checkValidTime = day => {
    ...
    start =
      parseInt(start[0], 10) * 600 +
      parseInt(start[1], 10) * 60 +
      parseInt(start[2], 10) * 10 +
      parseInt(start[3], 10);
    end =
      parseInt(end[0], 10) * 600 +
      parseInt(end[1], 10) * 60 +
      parseInt(end[2], 10) * 10 +
      parseInt(end[3], 10);
  ...
  const handleSubmit = event => {
    ...
    for (const day in state.Regimen_Week) {
      ...
      var start =
        state.Regimen_Week[day].startTime === ''
          ? ''
          : state.Regimen_Week[day].startTime.replace(':', '');
      var end =
        state.Regimen_Week[day].endTime === ''
          ? ''
          : state.Regimen_Week[day].endTime.replace(':', '');
      rw.push([start, end]);
    }
    if (state.Duration < 0) {
      return;
    ...
    fetch(
      'https://surzqe7m06.execute-api.us-west-1.amazonaws.com/521_addSchedule_stage',
      {
        method: 'POST',
        headers: {
