...
class CardPage extends React.Component {
  ...
  render() {
    return (
      <Page title={this.state.patient_list_key && this.state.patient_list_key[this.state.pid] && this.state.patient_list_key[this.state.pid].User_Name  + "'s devices"}>
        <Row>
          <Col md={12}>
            <Card>
                ...
                <UncontrolledButtonDropdown className="m-1">
                  ...
                  <DropdownMenu>
                    ...
                    Array.isArray(this.state.patient_list) && this.state.patient_list.map(friend => {
                      return <DropdownItem disabled={this.state.pid == friend.User_ID?true:false} onClick={() => this.goredirect('doctor-devicelist', friend.User_ID)}>{friend.User_Name}</DropdownItem>
                    })}
